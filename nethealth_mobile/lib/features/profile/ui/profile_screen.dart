...
class ProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Fallbacks
    final fullName = profile?.fullName ?? user?.fullName ?? 'User Name';
    final email = profile?.email ?? user?.email ?? 'user@example.com';
    final avatarUrl = profile?.avatarUrl ?? user?.avatar;
              ...
              child: Column(
                children: [
                  ...
                  _buildSection(context, isDark, 'Personal', [
                    ...
                    _buildTile(isDark, Icons.health_and_safety_outlined,
                        'Health Information',
                        onTap: () =>
                            _showHealthInfoSheet(context, isDark, profile)),
                    if (user?.role.name == 'doctor')
                      _buildTile(
                          isDark, Icons.calendar_month_outlined, 'My Schedule'),
                  ]),
                  const SizedBox(height: 24),
                  _buildSection(context, isDark, 'Preferences', [
                    _buildTile(
                        isDark, Icons.notifications_outlined, 'Notifications'),
                    ListTile(
                      onTap: () {
  ...
  void _showHealthInfoSheet(
      BuildContext context, bool isDark, PatientProfileModel? profile) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _HealthInfoSheet(isDark: isDark, profile: profile),
    );
  }
}
...
class _HealthInfoSheet extends StatelessWidget {
  ...
  const _HealthInfoSheet({required this.isDark, this.profile});
        ...
        children: [
          ...
          Text('Health Information',
              style: TextStyle(
                  ...
                  color: isDark
                      ? AppColors.textPrimaryDark
                      : AppColors.textPrimaryLight)),
          Text('Your medical overview on file',
              style: TextStyle(
                  fontFamily: 'Inter',
                  fontSize: 13,
                  color: isDark
            ...
            children: [
              Expanded(
                  child: _HealthTile(
                      ...
                      label: 'Blood Type',
                      value: profile?.bloodType?.label ?? 'N/A',
                      isDark: isDark)),
              ...
              Expanded(
                  child: _HealthTile(
                      ...
                      label: 'Conditions',
                      value: (profile?.chronicConditions?.isNotEmpty ?? false)
                          ? 'Yes'
                          : 'None',
                      isDark: isDark)),
            ],
          ...
          if (profile?.chronicConditions != null &&
            ...
            Container(
              ...
              child: Text(profile!.chronicConditions!,
                  style: const TextStyle(
                      fontFamily: 'Inter',
                      fontSize: 13,
