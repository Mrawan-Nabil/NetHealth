...
<template>
    <div>
        <transition name="fade">
            <button v-if="isOpen" type="button" class="fixed inset-0 z-30 bg-slate-900/50 lg:hidden" @click="$emit('close')" />
        </transition>

        <aside
            class="fixed top-0 left-0 z-40 flex h-screen w-72 flex-col border-r border-slate-200 bg-[#fbfdfc] px-5 py-5 transition-transform duration-300 lg:translate-x-0 dark:border-slate-700 dark:bg-slate-950"
        ...
        >
            <div class="flex items-center justify-between">
                ...
                <button type="button" class="rounded-lg p-1 text-slate-500 hover:bg-slate-100 lg:hidden dark:text-slate-300 dark:hover:bg-slate-800" @click="$emit('close')">
            ...
            <div class="mt-5">
                <label class="relative block">
                    ...
                    <input
                        type="text"
                        placeholder="Search"
                        class="h-10 w-full rounded-full border border-teal-100 bg-white py-2 pr-4 pl-9 text-sm text-slate-700 outline-none transition-colors duration-200 focus:border-teal-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                    />
                </label>
            </div>
            ...
            <nav class="mt-6 space-y-1.5">
                ...
                <button
                    ...
                    :class="item.active ? 'bg-teal-50 text-teal-700 dark:bg-teal-500/15 dark:text-teal-300' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'"
