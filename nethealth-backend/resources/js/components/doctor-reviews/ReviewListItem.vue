    ...
    >
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <p class="flex items-center justify-end gap-1 text-sm text-[#64748B] dark:text-[#94A3B8]">
                    <svg class="h-3.5 w-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path
                            ...
                            stroke-width="1.8"
                            d="M12 7v5l3 3M3.5 12a8.5 8.5 0 1 0 17 0 8.5 8.5 0 0 0-17 0Z"
                        />
                    </svg>
                    {{ item.date }}
                </p>
            ...
            <div class="flex shrink-0 flex-wrap items-center gap-2">
                ...
                <BaseButton v-if="item.status === 'Pending'" variant="success" size="sm" @click="$emit('view-files', item.patientId)"
                    >View Files</BaseButton
                >
                <template v-else>
                    <BaseButton variant="secondary" size="sm" @click="$emit('view-details', item.patientId)">View Details</BaseButton>
                    <BaseButton variant="ghost" size="sm" @click="$emit('edit-review', item.patientId)">Edit Review</BaseButton>
                </template>
            </div>
        </div>
    </article>
