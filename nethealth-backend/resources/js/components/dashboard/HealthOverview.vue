    ...
    >
        ...
        <div class="space-y-3">
            <div :class="isDark ? 'bg-[#0F172A]' : 'bg-[#F8FAFC]'" class="smooth-transition flex items-center justify-between rounded-lg p-4">
                ...
                <span :class="isDark ? 'text-[#F8FAFC]' : 'text-[#111827]'" class="text-sm font-semibold">{{ patient.bloodType }}</span>
            </div>

            <div :class="isDark ? 'bg-[#0F172A]' : 'bg-[#F8FAFC]'" class="smooth-transition flex items-center justify-between rounded-lg p-4">
                <span :class="isDark ? 'text-[#94A3B8]' : 'text-[#6B7280]'" class="text-sm font-medium">Allergies</span>
                <span :class="isDark ? 'text-[#F8FAFC]' : 'text-[#111827]'" class="text-sm font-semibold">{{ patient.allergies }}</span>
            </div>

            <div :class="isDark ? 'bg-[#0F172A]' : 'bg-[#F8FAFC]'" class="smooth-transition flex items-center justify-between rounded-lg p-4">
                <span :class="isDark ? 'text-[#94A3B8]' : 'text-[#6B7280]'" class="text-sm font-medium">Chronic Conditions</span>
                <span :class="isDark ? 'text-[#F8FAFC]' : 'text-[#111827]'" class="text-sm font-semibold">{{ patient.conditions }}</span>
            </div>
            ...
            <div :class="isDark ? 'bg-[#0F172A]' : 'bg-[#F8FAFC]'" class="smooth-transition rounded-lg p-4">
                ...
                <div class="flex items-center gap-3">
                    <img
                        :src="patient.doctor?.user?.avatar || 'https://i.pravatar.cc/150?img=47'"
                        :alt="patient.doctor?.user?.full_name"
                        class="smooth-transition h-10 w-10 rounded-full object-cover ring-2"
                        :class="isDark ? 'ring-[#334155]' : 'ring-gray-100'"
                    />
                    <div>
                        <p :class="isDark ? 'text-[#F8FAFC]' : 'text-[#111827]'" class="text-sm font-semibold">{{ patient.doctor?.user?.full_name }}</p>
                        <p :class="isDark ? 'text-[#94A3B8]' : 'text-[#6B7280]'" class="text-xs">
                            {{ patient.doctor?.specialty || 'Cardiologist' }}
                        </p>
                    </div>
                </div>
            </div>
