...
<template>
    <div
        ...
        class="animate-fade-in-up card-interactive group rounded-xl border p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
    >
        <!-- Header: Test Name & Status -->
        <div class="mb-4 flex items-start justify-between">
            <h3 :class="isDark ? 'text-[#F8FAFC]' : 'text-[#111827]'" class="pr-4 text-lg font-semibold transition-colors duration-300">
                {{ testResult.name }}
            </h3>
        ...
        <div class="mb-6 space-y-2">
            <div class="flex items-center gap-2 transition-all duration-300 group-hover:translate-x-1">
                <svg
                    class="h-4 w-4 text-[#14B8A6] transition-transform duration-300 group-hover:rotate-12"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                >
        ...
        <div class="flex justify-end">
            <button
                ...
                :disabled="isDisabled"
                :class="[
                    'rounded-full px-6 py-2 text-sm font-medium transition-all duration-300 hover:scale-105 active:scale-95',
                    isDisabled
                        ? isDark
                            ? 'cursor-not-allowed bg-[#334155] text-[#94A3B8]'
                            : 'cursor-not-allowed bg-gray-200 text-gray-400'
                        : isDark
                          ? 'bg-[#14B8A6] text-white shadow-lg shadow-teal-500/20 hover:bg-[#2DD4BF] hover:shadow-xl hover:shadow-teal-500/30'
                          : 'bg-[#14B8A6] text-white hover:bg-[#0D9488] hover:shadow-lg',
                ]"
            >
