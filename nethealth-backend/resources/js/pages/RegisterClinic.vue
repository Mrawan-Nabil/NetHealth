...
const STEPS = ['Admin Account', 'Admin Details', 'Clinic Profile'];

const STEP_FIELDS = [
    ['firstName', 'secondName', 'email', 'password', 'confirmPassword'],
    ['phone', 'dateOfBirth', 'gender', 'governorate', 'nationalId'],
    ['clinicName', 'clinicType', 'clinicPhone', 'clinicAddress', 'clinicGovernorate', 'licenseNumber', 'commercialRegistrationNumber', 'taxId'],
];
...
const CLINIC_TYPE_OPTIONS = [
    { value: 'single_specialty', label: 'Single-Specialty Clinic (عيادة تخصصية)' },
    { value: 'polyclinic', label: 'Polyclinic / Medical Center (مجمع طبي)' },
    { value: 'specialized_center', label: 'Specialized Center (مركز متخصص)' },
];
...
const clinicSchema = z
    .object({
        ...
        clinicGovernorate: z.string().min(1, 'Please select clinic governorate'),
...
const { validateField, setFieldError } = useForm({
    validationSchema: toTypedSchema(clinicSchema),
    initialValues: {
        firstName: '',
        secondName: '',
        email: '',
...
// Clinic Fields
const { value: clinicName, errorMessage: clinicNameError } = useField('clinicName', undefined, fieldOpts);
const { value: clinicType, errorMessage: clinicTypeError } = useField('clinicType', undefined, fieldOpts);
const { value: clinicPhone, errorMessage: clinicPhoneError } = useField('clinicPhone', undefined, fieldOpts);
const { value: clinicAddress, errorMessage: clinicAddressError } = useField('clinicAddress', undefined, fieldOpts);
const { value: clinicGovernorate, errorMessage: clinicGovernorateError } = useField('clinicGovernorate', undefined, fieldOpts);
const { value: licenseNumber, errorMessage: licenseNumberError } = useField('licenseNumber', undefined, fieldOpts);
const { value: commercialRegistrationNumber, errorMessage: commercialRegistrationNumberError } = useField(
    'commercialRegistrationNumber',
    undefined,
    fieldOpts,
);
...
function onSubmit() {
    const payload = {
        ...
        clinic_type: clinicType.value,
        clinic_phone: clinicPhone.value,
        clinic_address: clinicAddress.value,
        clinic_governorate: clinicGovernorate.value,
        license_number: licenseNumber.value,
        commercial_registration_number: commercialRegistrationNumber.value,
        tax_id: taxId.value,
        verification_documents: verificationFiles.value,
    };

    router.post('/register/clinic', payload, {
        onError: (errors) => {
            ...
            // Map Clinic Errors
            if (errors.clinic_name) setFieldError('clinicName', errors.clinic_name);
            if (errors.clinic_type) setFieldError('clinicType', errors.clinic_type);
            if (errors.clinic_phone) setFieldError('clinicPhone', errors.clinic_phone);
            if (errors.clinic_address) setFieldError('clinicAddress', errors.clinic_address);
            if (errors.clinic_governorate) setFieldError('clinicGovernorate', errors.clinic_governorate);
        ...
        <Transition name="page-fade" appear>
            <div class="space-y-6">
                <div class="text-center">
                    <h1 class="text-2xl font-bold text-gray-900">Register Clinic</h1>
                    <p class="mt-1 text-sm text-gray-600">Create your admin account and register your facility</p>
                </div>
                            ...
                            <div :key="stepIndex">
                                ...
                                <form v-else-if="stepIndex === 1" class="grid grid-cols-2 gap-4" @submit.prevent>
                                    ...
                                    <BaseInput
                                        v-model="nationalId"
                                        label="National ID"
                                        placeholder="Admin's National ID"
                                ...
                                <form v-else-if="stepIndex === 2" class="grid grid-cols-2 gap-4" @submit.prevent>
                                    <BaseInput
                                        v-model="clinicName"
                                        label="Clinic Name"
                                        placeholder="e.g. Al-Shefaa Clinic"
                                    ...
                                    <BaseSelect
                                        ...
                                        class="col-span-2"
                                    >
                                        <option v-for="opt in CLINIC_TYPE_OPTIONS" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                                    </BaseSelect>

                                    <div class="col-span-2">
                                        <PhoneInput
                                            v-model="clinicPhone"
                                            label="Clinic Landline/Phone"
                                    ...
                                    <BaseSelect
                                        v-model="clinicGovernorate"
                                        label="Clinic Governorate"
                                        placeholder="Select governorate"
                                        :error="clinicGovernorateError"
                                        class="col-span-2"
                                    >
                                        <option v-for="opt in GOVERNORATE_OPTIONS" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                                    </BaseSelect>
                                    ...
                                    <BaseInput
                                        ...
                                        placeholder="Ministry of Health License"
                                    ...
                                    <BaseInput
                                        ...
                                        :error="commercialRegistrationNumberError"
                                        class="col-span-1"
                                    />
                                    ...
                                    <div class="col-span-2">
                                        <label class="mb-1.5 block text-sm font-medium text-gray-700"
                                            >Verification Documents (MoH License, Admin ID)</label
                                        >
                                        <input
                                            ref="dropzoneInput"
                                            type="file"
